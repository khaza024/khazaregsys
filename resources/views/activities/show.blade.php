<x-app-layout :title="$activity->title">
    <article class="w-full col-span-4 py-5 mx-auto mt-10 md:col-span-3" style="max-width:700px">
        <img class="w-full my-2 rounded-lg" src="{{ $activity->getThumbnailUrl() }}" alt="{{ $activity->slug }}">
        <h1 class="text-4xl font-bold text-left text-gray-800">
            {{ $activity->title }}
        </h1>
        <div class="flex items-center justify-between mt-2">
            <div class="flex items-center py-5">
                <x-activities.author :author="$activity->author" size="md" />
                <span class="text-sm text-gray-500">| {{ $activity->getReadingTime() }} min read</span>
            </div>
            <div class="flex items-center">
                <span class="mr-2 text-gray-500">{{ $activity->published_at->diffForHumans() }}</span>
